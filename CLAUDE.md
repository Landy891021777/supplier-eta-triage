...
| **5 重跑評估與種子快取（會用 Gemini 額度，執行前先問使用者）** | **下一步** |
| 6 決策 19、合併到 `main`、切換公開網址 | 待辦 |

**Task 5 要清掉的待補標記**：README「解析」「檢索」兩張表與結論句、`docs/部署指引.md` 檢查清單的
開 LLM 時 P1 數。用 `grep -rn "TBD-待" --include=*.md .`（排除 `docs/superpowers/`）檢查，必須沒有輸出。
回測數字在 Task 4 已離線重跑確認沒變，README 直接引用。
