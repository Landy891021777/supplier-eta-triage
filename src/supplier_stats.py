# -*- coding: utf-8 -*-
"""
供應商歷史表現：把 ERP 收貨紀錄變成物料企劃當下用得到的決策依據。

===========================  為什麼需要這一支  ===========================
歷史單據現在餵給兩件事：行動清單上的保守到料日估計，
以及 `src/backtest.py` 的時間切分回測。**沒有這一支，物料企劃在每天的
行動清單上，看不到任何歷史資訊。**

歷史資料真正的價值，是回答物料企劃看到一封延遲通知時心裡的那個問題：
...
它不宣稱知道「這一張單會怎樣」。物料企劃看得懂、也可以自己驗證，
而且不需要任何模型維護成本。
