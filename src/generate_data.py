...
FIXED_POS = [
    ...
    # 手寫案例 HC-010 依賴這張單：ERP 裡是單一排程行（20 片 @ 2026-09-30，
    # 尚未拆行），HC-010 的信是供應商「提議」拆成 8 片照原日期、12 片延到
    # 11/15——不能預先在 ERP 把它拆好，那樣信件就變成單純複述 ERP 已經
    # 知道的事，兩批都會被判成 no_change，看不出供應商延遲了 12 片
    # （見決策 19 的修正：提議拆批 vs. ERP 已拆批是兩件不同的事）。
    ("PO-2026-04188", "TG-Cu-90",      "SUP-T02", 20,   "2026-09-30", "2026-10-08", True,  1, 0.75, None),
]
